...
const auth = catchAsync(async (req, res, next) => {
    ...
    if (!apiKey) {
        return next(new AppError('API Key is missing or invalid. Please provide x-api-key header.', 401));
    }
    ...
    keyDoc.save().catch(err => console.error('Failed to update API key usage stats:', err));
